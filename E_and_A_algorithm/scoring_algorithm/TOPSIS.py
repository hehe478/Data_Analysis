...
def perform_topsis_analysis(matrix, row_type, row_best):
    """
    执行完整的TOPSIS分析，包括数据预处理、计算距离和最终得分。
...
def print_topsis_results(results):
    """
    格式化并打印TOPSIS分析的结果。

    参数:
    results(dict): 从 perform_topsis_analysis 函数返回的结果。
    """
    # 6. 输出最终结果
    print("\n--- TOPSIS 法计算结果 ---")
    print(f"每个指标的最优理想值 (Ideal Best): {[f'{val:.4f}' for val in results['ideal_best']]}")
    print(f"每个指标的最劣理想值 (Ideal Worst): {[f'{val:.4f}' for val in results['ideal_worst']]}")
    print(f"各评价对象到最优解的距离 (D+): {[f'{val:.4f}' for val in results['d_best']]}")
    print(f"各评价对象到最劣解的距离 (D-): {[f'{val:.4f}' for val in results['d_worst']]}")
    print("-" * 30)
    ...
    # normalized_scores = 100 * results['scores'] / np.sum(results['scores'])
    # print("\n--- 百分制得分 ---")
...
if __name__ == '__main__':
    ...
    # 调用核心函数执行TOPSIS分析，并获取结果
    analysis_results = perform_topsis_analysis(matrix_test, row_type_test, row_best_test)

    # 调用打印函数，展示分析结果
    print_topsis_results(analysis_results)